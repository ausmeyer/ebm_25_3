...
- A p-value threshold of .05 means we reject H0 when p < .05 and fail to reject when p >= .05.
:::
...
:::{.incremental}
- **A (power):** power concerns sample size and type II error risk, not matching strategy.
...
- Chi-square is the standard nonparametric test for association in categorical data.
...
- That absolute difference is 0.2%, or about 2 additional events per 1,000 treated.
...
- p > .05 does not prove no effect if power is low.
:::
...
:::{.incremental}
- Original small study accepted the null (“no effect”).
...
document.addEventListener('DOMContentLoaded', function() {
  const explanations = {
    ...
    2: `<p><strong>Correct Answer: B. confounding</strong></p>
        <p>Sibling matching controls shared environmental covariates and reduces confounding.</p>`,
